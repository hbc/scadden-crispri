...
We also noticed that there seem to be an separation of the samples based on
...
Before we do this, though, we have to combine the samples that are technical
...
`derived` to the sample metadata. Now we sum all of the counts for each
sample with the same derived name for the `Gecko A` library:
...
Now we are ready to rock. We'll dump out the filtered tables of counts and
the metadata to a file so we can import it later.

```{r cleanup}
a_samples = a_combined_samples
b_samples = b_combined_samples
dir.create("../processed")
save(b_counts, a_counts, b_samples, a_samples, file="../processed/cleaned.RData")
rm(list=ls())
```
